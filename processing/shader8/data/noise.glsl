...
uniform vec2 domainWarp; //x and y
uniform vec2 freqWarp; //lower and upper multiple
uniform float oWarp;
...
float evaluate(vec2 pos, float freq, vec2 oRange){ 
	...
	//v = 6*v/(rootLambda*gs);
	v= .01*v/(sqrt(density)+.01*sqrt(1.0/freq));
	return v;
}
...
void main(void){
	...
	vec2 domainOffset = readFlags.xy * domainWarp.xy*2.0*(colorIn.xy-vec2(.5));
	float freqMult = 1.0;
	if(readFlags.z > 0)
		freqMult = mix(freqWarp.x, freqWarp.y, colorIn.z);
	float orientation = harmonic.z + PI*(colorIn.w-.75)* readFlags.w * oWarp; //WHY .75??
	vec2 oRange = vec2(orientation-.5*harmonic.w, orientation+.5*harmonic.w);
	
	vec2 pos = gl_FragCoord.xy+origin.xy;
	float v = octaves(pos+domainOffset, harmonic.x*freqMult, oRange, harmonic.y);
