...
vec2 octaves(vec2 pos, float freq, vec2 oRange, float num){
	...
	for(int oct = 0; oct<OCTAVE_CAP; oct++){
		if(oct>=num) break;
		float potential_left = evaluate(pos+vec2(-1.0,0.0), freq*pow(2.0,oct), oRange)*s;
		float potential_right = evaluate(pos+vec2(1.0,0.0), freq*pow(2.0,oct), oRange)*s;
		float potential_up = evaluate(pos+vec2(0.0,1.0), freq*pow(2.0,oct), oRange)*s;
		float potential_down = evaluate(pos+vec2(0.0,-1.0), freq*pow(2.0,oct), oRange)*s;
