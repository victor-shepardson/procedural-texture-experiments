#define PROCESSING_COLOR _SHADER
...
float evaluate(vec2 pos, int i, int j, float freq, vec2 orange){ //i is instance for warping, j is octave
...
float octaves(vec2 pos, int i, float freq, vec2 orange){
	...
	for(int oct = 0; oct<OCTAVE_CAP; oct++){
		if(oct>=num) break;
		sigma += evaluate(pos, i, oct, freq, orange)*s;
		norm+=s;
		s/=2.0;
	}
	return sigma/norm;
}
float octaves_warp(vec2 pos, int i, float freq, vec2 orange){
	int num = int(harmonic[i].y);
	float sigma=0.0;
	float s=2.0/harmonic[i].x*pow(2,-num);
	for(int c = 0; c<OCTAVE_CAP; c++){
		int oct = num-c-1;
		if(oct<0) break;
		sigma = evaluate(pos+s*vec2(sigma, 0), i, oct, freq, orange);
		s*=2.0;
	}
	return sigma;
}
void main(void){
	...
	float nx = octaves(pos, 0, harmonic[0].x, harmonic[0].zw);
	float ny = octaves(pos, 1, harmonic[1].x, harmonic[1].zw);
	float v = octaves(pos+vec2(nx*warp.x, 0), 2, (warp.x*nx+1)*harmonic[2].x, mix(harmonic[2].zw,vec2(ny*PI),warp.y));
	v=v*.5+.5;
	vec3 c = vec3(v,v,v);
